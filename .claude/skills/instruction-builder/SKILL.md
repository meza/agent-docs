---
name: Agent/Skill Instruction Builder
description: How to write instructions for skills and agents. Use this when crafting instruction sets that are targeted for reasoning agents
---

# How to Craft a Good System Message

A system message is an architectural document. It defines the cognitive environment an agent will inhabit: the mindset, boundaries, and reasoning discipline that make the agent's behavior clear, reliable, and aligned.

The system message does not perform the task. It establishes the conditions under which the task can be performed with clarity and integrity. The craft is the ability to translate user intent into a coherent operational identity—an explicit statement of what an agent is, what it is for, and how it should think while performing its role.

A good system message earns authority through precision, not verbosity.

## The Core Design Goal

The goal is coherence.

Coherence means the agent can follow its mission without contradiction, silent prioritization, or invented authority. It also means the system message reads as a single, continuous line of thought: purpose establishes direction, boundaries define limits, reasoning discipline governs decisions, and output discipline makes results usable.
...
Crafting a system message includes defining a persona, because persona is part of the cognitive environment.
...
A system message should speak directly to the agent being defined in clear declarative language, typically of the form "You are X. Your purpose is...". This anchors identity and makes the persona operational rather than implied.

System messages should avoid self-referential or first-person phrasing. The system message is not a conversation with the user; it is the agent's blueprint. Direct address keeps identity stable and reduces ambiguity about authority.

## Focus and Scope Integrity

A system message should define one agent with one clearly bounded role.
...
Ambiguity that would distort an agent's identity, purpose, or reasoning model should not be accepted. The commitment is to coherence: the agent must be able to follow its mission without contradiction, and the system message should prevent role creep by design.

## Choosing the Right Level of Abstraction

Default to high-level structural guidance.

A system message should define purpose, boundaries, reasoning modes, and communication expectations without slipping into implementation detail. Reasoning agents require frameworks, not scripts. Frameworks generalize across situations; scripts collapse when conditions change.
...
Crafting a system message is a design activity. The process is not bureaucracy; it is how coherence is protected from ambiguity and overload.
...
Drafting produces a structured system message addressed directly to the agent.
...
Iteration refines the system message through user feedback while preserving unity.

The system message should remain one purpose, one identity, one chain of reasoning. Revision should remove redundancy, tighten causality, and prevent drift into excessive detail. The goal is not to grow the document; it is to stabilize it.

### Delivery

Delivery is a discipline of form.

When the user confirms completion, the output should be the system message alone, enclosed in a single fenced code block, with no commentary or meta-notes inside the block. This preserves normative clarity and prevents downstream confusion about what is instruction versus explanation.

## Reasoning Framework to Embed

A good system message defines how the agent should think, not what it should think.
...
Verification is a guardrail against plausible-sounding output that does not actually follow from what is known. It also reduces misalignment by ensuring the agent's reasoning remains anchored to the user's purpose and the system message's constraints.

### Bias Control

Every system message imposes design biases, because it privileges certain goals and behaviors.

Those biases should be acknowledged and monitored. The system message should require the agent to watch for overreach and unwarranted inference—especially where the role's purpose might tempt the agent into confidence that is not supported by evidence.

### Error Handling

When data are missing or contradictory, the system message should require the agent to pause, clarify, or proceed conservatively while marking assumptions.
...
The system message should encourage reflection before responding.
...
Do not insert first-person statements into system messages. System messages are blueprints addressed to the agent; first-person phrasing blurs the locus of authority and invites interpretive drift.
...
Before delivering a system message, it should be audited for coherence.
...
A complete system message should satisfy the following conditions:
...
- When data are missing or contradictory, the system message requires clarification or conservative proceeding with assumptions marked.
- No first-person phrasing appears in the system message.
...
- The system message doesn't contain any smart-quotes or other non-ascii punctuation
